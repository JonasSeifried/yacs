...
| Pairing | Until 0.3.x: a shared phrase (generated by default, Argon2id). Planned (section 8): **spaces** joined with one-time **invites** (code, link or QR), no phrase, no logins. A public relay at `app.yacs.jonasseifried.com` next to self-hosting; premium per space through an **account key**. |
...
- `app.yacs.jonasseifried.com`, run by the author (`yacs.jonasseifried.com` is kept for a homepage with the privacy policy and imprint; the relay serves the phone app at `/`, so they can't share a name): for now on the author's home server, which already serves other sites behind nginx with port forwarding and a certificate script, so it's `compose.nginx.yaml` with `YACS_PUBLIC=true` and one more nginx site. `YACS_MAX_DISK` a few GB, not the default 25. Move to a VPS (e.g. Hetzner) when there are users who'd notice downtime or once premium exists; clients only know the domain and clips expire within the hour, so moving is a DNS change plus copying `accounts.json`. The author's own devices use the same relay with the owner's key, so problems show up there first.
...
| **9: Public relay** (code ✅; not live yet) | People without a server | Public mode (per-IP limits, free plan); account keys + channel registration (self-hosted token becomes the owner's key); per-space limits in `/limits`; onboarding "free relay or your own"; `app.yacs.jonasseifried.com` live with privacy policy + Impressum | A new user installs the app and syncs a phone without setting anything up |
