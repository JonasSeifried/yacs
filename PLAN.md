...
- **Pairing via QR:** desktop Settings → *Pair a phone* shows `https://your-server/#pair=v1.<channel id>.<key>[&token=…]`. It carries the *derived* pairing (`Pairing::to_secret` in `yacs-core`), not the phrase: the desktop never stores the phrase, and the phone skips Argon2id. Everything is in the URL fragment, which browsers never send, so it never reaches the server or its logs; the app removes it from the address bar as soon as it's read. Typing the phrase works too (Argon2id in WASM, about a second on a phone).
...
| **4: PWA + release (v1.0)** 🚧 (v0.1.0 released; relay runs on a VPS behind nginx; phone over HTTPS still to test) | Mobile + ship | `yacs-wasm`, mobile UI, embedded PWA, QR pairing, Dockerfile + compose (Caddy or nginx), signed desktop builds, updater | A phone can pair via QR and copy/send; `docker compose up` works on a VPS |
