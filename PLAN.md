...
2. `↑/↓`: select a clip. `Enter` or `Ctrl+C`: Rust writes all formats of the selected clip to the clipboard. Spotlight shows "Copied" and hides after 5 s, or right away on `Esc`, and focus returns to the previous app. In those 5 s, `Ctrl+V` also just hides instead of sending the clip straight back. Double-click copies too.
3. `Ctrl+V`: the keydown is intercepted. Rust reads the native clipboard (text, HTML, RTF, image; PNG is passed through untouched), encrypts, POSTs with the TTL from the dropdown, shows "Sent", then the window hides. A single copied image file (Finder/Explorer) is sent as that image: PNG, JPEG, GIF and WebP as they are, BMP and TIFF converted to PNG, up to 64 MB. Other files are refused with a clear message until P2P in v2.
