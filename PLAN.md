...
- **Changes in phase 8 (section 8):** the phrase and Argon2id go. A new space's root is 32 random bytes; the HKDF step to channel id and key stays.
...
- **CLI:** `yacs join <code|link>` (`yacs pair` stays as an alias), `yacs invite` (prints a link and a code, waits while the code is live), `yacs spaces`, `yacs space rename OLD NEW`, `--space NAME` on send/recv/list, `yacs space export`, `yacs account add KEY`, `yacs space upgrade`.
